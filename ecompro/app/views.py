...
class ProductView(View):
    def get(self, request):
        tshirts = Product.objects.filter(category='TW')
        mobile = Product.objects.filter(category='M')
        jeans = Product.objects.filter(category='BW')
        return render(request, 'app/home.html', {'tshirts': tshirts, 'mobile': mobile, 'jeans': jeans})
...
class CustomerRegistration(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            messages.success(request,"Congratulations!!! Regsitered")
            form.save()
        return render(request, 'app/customerregistration.html', {'form': form})
...
class ProfileView(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            reg.save()
            messages.success(request,"Congratulation, proile updated")
        return render(request, 'app/profile.html',{'form':form,'active':'btn-primary'})

@login_required
def address(request):
    user_address=Customer.objects.filter(user=request.user)
    return render(request, 'app/address.html',{'add':user_address})
